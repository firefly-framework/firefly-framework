{% macro where_clause(c, attribute_macro, value_macro, ids) %}
    ...
    {% else %}
        {%- if c is criteria -%}
            {% if c.lhv is criteria %}({% endif %}{{ where_clause(c.lhv, attribute_macro, value_macro, ids) }}{% if c.lhv is criteria %}){% endif %}
        {%- endif -%}
        ...
        {% if c is criteria %}
            ...
            {% elif c.op == 'startswith' %}
                like CONCAT({{ where_clause(c.rhv, attribute_macro, value_macro, ids) }}, "%")
            {% elif c.op == 'endswith' %}
                like CONCAT("%", {{ where_clause(c.rhv, attribute_macro, value_macro, ids) }})
            {% elif c.op == 'is' %}
        ...
        {%- if c is criteria and c.op not in ['startswith', 'endswith', 'is'] -%}
            {% if c.rhv is criteria %}({% endif %}{{ where_clause(c.rhv, attribute_macro, value_macro, ids) }}{% if c.rhv is criteria %}){% endif %}
        {%- endif -%}
        ...
        {% if c is not criteria %}
            ...
            (
                {% for i in c %}
                {{ value_macro(i, ids) }}{% if not loop.last %},{% endif %}
                {% endfor %}
            )
            {%- else -%}
                {{ value_macro(c, ids) }}
...
{% macro default_value_macro(v, ids) %}
    {{ v }}
{% endmacro %}